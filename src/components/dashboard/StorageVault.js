...
const StorageVault = ({ isOpen, setIsOpen, handleShowSnackbar, setIsLoading, isValidated, driveUser }) => {
    ...
    const manageFile = async (nameChange=false) => {
        ...
        const dataToPatch = nameChange ?? draggedFile;
            ...
            <DialogTitle className='flex items-center justify-between'>
                ...
                <Box className='w-1/3 h-12 flex flex-col items-start justify-between'>
                    ...
                    <Box className='relative w-full h-4 bg-slate-200 rounded-3xl overflow-hidden'>
                        <Box className={`absolute w-[${Math.floor(totalStorage)}%] left-0 top-0 h-full bg-blue-500 rounded-3xl`}></Box>
                    </Box>
                </Box>
            </DialogTitle>
            <DialogContent>
                <Box className='w-full h-full overflow-auto' sx={{scrollbarWidth : 'none'}}>
                    <Box className='w-full'>
                        ...
                        <Button key={index} startIcon={<IconButton disabled><FolderRounded color='action' sx={{margin : '0 10%'}} /></IconButton>} 
                                endIcon={<IconButton onClick={(event) => handleClick(event, index)}><MoreVertRounded/></IconButton>}
                                sx={{ textTransform: 'capitalize', border : 'solid 1px lightgrey', fontSize : '18px', margin : '0 1% 1% 0' }} color='inherit'
                            ...
                            >
                                <MenuItem onClick={() => handleClose(index)}><OpenInNewRounded color='action' sx={{marginRight : '10px'}}/> Open</MenuItem>
                                <MenuItem onClick={() => handleClose(index)}><DriveFileRenameOutlineRounded color='action' sx={{marginRight : '10px'}}/> Rename</MenuItem>
                                <MenuItem onClick={() => handleClose(index)}><DeleteRounded color='action' sx={{marginRight : '10px'}}/> Delete</MenuItem>
                            </Menu>
                        </Button>)}
                    </Box>
                    ...
                    <Box className='w-full max-h-[34rem]'>
                        <Box className='w-full h-16 pt-2 flex items-start justify-between'>
                            <Typography color='grey' fontSize={18} sx={{marginBottom : '10px'}}>Files</Typography>
                            <Box className='w-1/3'>
                            <Typography sx={{fontFamily : 'sans-serif'}}>10 MB of {filesStorage} MB Used by Files</Typography>
                            <Box className='relative w-full h-3 bg-slate-200 rounded-3xl overflow-hidden'>
                                <Box className={`absolute w-[${Math.floor(filesStorage)}%] left-0 top-0 h-full bg-blue-500 rounded-3xl`}></Box>
                            </Box>
                            </Box>
                        </Box>
        ...
        <Dialog open={createFolder} maxWidth='lg' sx={{zIndex : '810'}}>
            ...
            <DialogContent className='w-[40rem] h-[40rem]'>
                <VerticalLinearStepper setCreateFolder={setCreateFolder} handleShowSnackbar={handleShowSnackbar} 
                                        ...
                                        setFileCreate={setFileCreate} fetchData={fetchData} />
            </DialogContent>
        </Dialog>
