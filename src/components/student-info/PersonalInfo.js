...
import { Box, Typography, Button, Link, IconButton, Tooltip, Badge, Dialog, DialogTitle, DialogContent, DialogActions, Checkbox, FormGroup, FormControlLabel } from '@mui/material';
import { CalendarMonthRounded, CancelRounded, CheckCircleRounded, DeleteForeverRounded, DescriptionRounded, DoneRounded, Email, LocationOnRounded, NotInterestedRounded, Phone, PhoneAndroid, SendRounded, WcRounded } from '@mui/icons-material';
import { StudentsContext } from '../api/students';
import { DateTime } from '../date-time';

const PersonalInfo = ({ user, stdId, student_Details, location, access, status, handleShowSnackbar, setMessageForm, fetchStdData, handleClose, JoiningDate }) => {
        ...
        <Box className="w-full h-[70%] flex flex-row items-start justify-start">
            <Box className="w-full h-full flex flex-row items-start pl-10">
                <Box className="w-1/3 h-96 flex flex-col items-center justify-evenly">
                    <img src={(!student_Details.Image || student_Details.Image === 'N/A') ? "/images/Empty-Men-Icon.png" : student_Details.Image} alt='' width="200px" className='border-[1px] border-gray-300 h-[200px] rounded-full object-fill'/>
                ...
                <Box className="flex w-1/2 flex-col items-center justify-end ml-32">
                    ...
                    <Box className='w-full grid grid-cols-2 gap-7 mt-10'>
                        ...
                        <Typography><Email className='text-gray-500' /> &nbsp;{student_Details.Email}</Typography>
