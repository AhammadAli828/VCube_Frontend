  ...
  return (
    ...
    {data.map((item,idx)=>(
        <Accordion expanded={view === idx} key={idx} onChange={()=>setView(view === idx ? null : idx)} className='w-full border-t-[1px] border-gray-300'>
          <AccordionSummary><SchoolRounded sx={{fontSize : '30px',marginRight : '10px'}} className='text-gray-500' /><Typography variant='h5'>{(idx === 0) ? '10th Standard' : (idx === 1) ? '12th Standard' : (idx === 2) ? 'Degree' : 'PG'} Details</Typography></AccordionSummary>
          <AccordionDetails>
